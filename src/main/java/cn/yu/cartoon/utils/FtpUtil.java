...
public class FtpUtil {
     ...
     **/
    public boolean upload(String host, int port, String username, String password, String sourceFilePath, String remoteDirPath, String newRemoteDirName) throws IOException {
        ...
        //如果是文件夹
        //将文件夹存入ftp服务器
        refreshFileList(sourceFile, (remoteDirPath + "/" + newRemoteDirName), ftpClient);
     ...
     **/
    private void refreshFileList(File sourceFile, String remoteDirPath, FTPClient ftpClient) {
        ...
        for (File file : files) {
            ...
            if (file.isDirectory()) {
                //是文件夹,则递归
                //递归文件夹！！！
                refreshFileList(file, remotePathCurrent, ftpClient);
            } else {
                //是文件,则执行上传到FTP方法
                process(file, remotePathCurrent, remoteDirPath, ftpClient);
            }
        }
    }
     ...
     * @param ftpClient ftp连接对象
     **/
    private void process(File sourceFile, String remoteFilePath, String remoteDirPath, FTPClient ftpClient){
